            ...
            <div class="label">
                <el-popover ref="tag" placement="top-start" width="150" trigger="click">
                    <ul class="tag-list-wrap">
                        <li v-for="(item, index) in tags" :key="index" @click="selectTag(item)">{{item.tagName}}</li>
                    </ul>
                </el-popover>
                <img src="../assets/tag.png" height="30" width="30" v-popover:tag>
                <el-tag style="margin: 0 3px 0 3px;" v-for="(item, index) in list" :closable="true" type="success" :key="index" :close-transition="false" @close="handleClose(tag)">
                    {{item.tagName}}
                </el-tag>
            </div>
